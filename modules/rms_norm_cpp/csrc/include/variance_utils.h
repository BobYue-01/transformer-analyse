...
template <typename T>
C10_ALWAYS_INLINE void AddVarianceVec(
    int64_t m0_add,
...
UpdateVarianceVec(
  ...
  AddVarianceVec(m0, /* m1_vec, */ m2_vec, m0_stk0, /* m1_stk0, */ m2_stk0);
}
...
UpdateVarianceVec(
  ...
  for (const auto j : c10::irange(m0)) {
    ...
    m2_fvec0 += x_fvec0 * x_fvec0;
    m2_fvec1 += x_fvec1 * x_fvec1;
  }
  AddVarianceVec(m0, /* m1_fvec0, */ m2_fvec0, m0_stk0, /* m1_stk0, */ m2_stk0);
  AddVarianceVec(m0, /* m1_fvec1, */ m2_fvec1, m0_stk0, /* m1_stk0, */ m2_stk0);
}

// Compute rowwise variance by cascade sum to improve numerical stability.
// https://en.wikipedia.org/wiki/Pairwise_summation
template <typename T, int64_t kMaxDepth>
opmath_t<T> RowwiseVarianceImpl(const T* X, int64_t N, int64_t ddof = 0) {
  ...
  for (const auto i : c10::irange(m)) {
    ...
    //   return result;
    // })();
    UpdateVarianceVec(m0, X_ptr, /* c_vecs, */ m0_stk[0], /* m1_stk[0], */ m2_stk[0]);
  ...
  for (const auto i : c10::irange(kAccVecSize)) {
    AddVariance(m0_add, /* m1_arr[i], */ m2_arr[i], m0, /* m1, */ m2);
  }

  return m2 / static_cast<math_t>(N - ddof);
}

template <typename T>
opmath_t<T> RowwiseVariance(const T* X, int64_t N, int64_t ddof = 0) {
  using Vec = vec::Vectorized<T>;
  ...
  const int64_t depth = utils::CeilLog2(m);
  if (depth <= 4) {
    return RowwiseVarianceImpl<T, 4>(X, N, ddof);
  } else if (depth <= 8) {
    return RowwiseVarianceImpl<T, 8>(X, N, ddof);
  } else if (depth <= 16) {
    return RowwiseVarianceImpl<T, 16>(X, N, ddof);
  } else if (depth <= 32) {
    return RowwiseVarianceImpl<T, 32>(X, N, ddof);
  } else {
    return RowwiseVarianceImpl<T, 64>(X, N, ddof);
  }
}
