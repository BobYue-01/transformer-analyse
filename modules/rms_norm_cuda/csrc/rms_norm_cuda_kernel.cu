...
__global__ void RowwiseVarianceCUDAKernel(
    int64_t N,
    T_ACC eps,
    const T* X,
    T_ACC* rstd) {
  using VarianceType = VarianceData<T_ACC, int64_t>;
  using VarianceOp =
      VarianceOps<T_ACC, T_ACC, int64_t /*, thrust::pair<T_ACC, T_ACC> */>;

  __shared__
      typename std::aligned_storage<sizeof(VarianceType), alignof(VarianceType)>::
          type val_shared[C10_WARP_SIZE];
  VarianceType* val_shared_ptr = reinterpret_cast<VarianceType*>(val_shared);

  const int64_t i = blockIdx.x;
  VarianceOp variance_op = {/*correction=*/0, /*take_sqrt=*/false};
  VarianceType val(/* 0, */ 0, 0, 0);

  for (int64_t j = threadIdx.x; j < N; j += blockDim.x) {
    const int64_t index = i * N + j;
    val = variance_op.reduce(val, static_cast<T_ACC>(X[index]), index);
  }
  val = cuda_utils::BlockReduce(
      val,
      variance_op,
      /*identity_element=*/VarianceType(/* 0, */ 0, 0, 0),
      val_shared_ptr);

  if (threadIdx.x == 0) {
    T_ACC m2;
    /* thrust::tie( */ m2 /*, m1) */ = variance_op.project(val);
...
struct VarianceDataRmsNorm{
  float sigma2;
  float count;
  C10_HOST_DEVICE VarianceDataRmsNorm(): sigma2(0.f), count(0.f){}
  C10_HOST_DEVICE VarianceDataRmsNorm(float sigma2, float count): sigma2(sigma2), count(count) {}
};

template<typename U> __device__
VarianceDataRmsNorm cuVarianceOnlineSum(
  const U val,
  const VarianceDataRmsNorm& curr_sum)
{
  return {curr_sum.sigma2 + val * val, curr_sum.count + 1.f};
}

__device__
VarianceDataRmsNorm cuVarianceCombine(
  const VarianceDataRmsNorm dataB,
  const VarianceDataRmsNorm dataA
) {
...
template<typename T>
__device__ VarianceDataRmsNorm compute_stats(
  ...
  float * buf
    ...
    if (blockDim.y > 1) {
      ...
      __syncthreads();
      return VarianceDataRmsNorm{sigmabuf[0], 0.f};

    } else {
      return VarianceDataRmsNorm{WARP_SHFL(wd.sigma2,0)/float(N), 0.f};
    }
}
...
__device__ __inline__ void vectorized_rms_norm_kernel_impl(
  ...
  T_ACC* rstd,
  T* Y){
    extern __shared__ float s_data[]; //if we made smem VarianceDataRmsNorm type, there would be bank conflicts,
    //as one thread would have to write 3 consecutive floats
    auto i1 = blockIdx.x;
    const T * block_row = X + i1 * N;
    VarianceDataRmsNorm wd = compute_stats(block_row, N, s_data);
...
void RmsNormKernelImplInternal(
    ...
    Tensor* Y,
    Tensor* rstd) {
  // assumes input, gamma and beta are of proper shape, this was checked in _check_rms_norm_inputs
  ...
  RowwiseVarianceCUDAKernel<T, T_ACC>
